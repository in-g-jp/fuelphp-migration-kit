...
```
> **限定 config の盲点**: 列挙したルールしか見ないので、**Rector にルールが無い非互換（例 `curl_close`）は出ない**。それは手順3の grep で取る。
...
> シグネチャ不変・呼んでも動く・**Rector に該当ルールが無い**ため 2 を素通りし実機ログにしか出ないが、
> UPGRADING-8.5 に明記されているので **grep に列挙すれば静的に取れる**。
