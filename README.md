...
| 業務データ（seed） | 全導線を実データで踏む | **ここで作って投入まで済ませる**。下記いずれか。**PII なし**・マスタ系と業務系でファイル分割。生成スクリプト（例 `docker/db/gen_dummy_seed.php`）→ `initdb/*.sql` に吐き、fresh な `compose up` で自動投入できる形に |
...
> 本番ダンプがあれば `SHOW COLUMNS` の集合を本番 DDL と `comm` で差分照合し、欠けた列は補完 migration を足す（型・位置は本番 DDL で確認）。この一致確認は seed 投入前に済ませる。
...
> **「制約が通る ≠ 動く」。** インストール済み依存の php 制約は緩く 8.x を許容しがちで、`why-not` が阻害なしでも、
> 実際の壁は依存制約ではなく **framework 自身のコード**（curly brace offset・required-after-optional 等＝手順2/3 で出る）。
...
**Rector は「全セット有効化 + 近代化ルールを skip」で回す。allow-list は使わない。**
`withRules([...])` で移行必須ルールだけ列挙する方式は**脆く取りこぼす**:
PHP セットには **設定済みの汎用ルール**（`RemoveFuncCallRector` が `curl_close` を削除、`RenameFunctionRector` が `mysqli_execute`→`mysqli_stmt_execute`、`RenameCastRector` が `(integer)`→`(int)` 等）が多数あり、これらは「移行必須」だが個別ルール名で列挙しづらい。
逆に **`withPhpSets(php85)` で全部 ON にし、`array()→[]` 等の cosmetic だけ `withSkip` で外す**。こうすれば設定済み/新規の移行必須ルールは default ON で必ず拾える:
```php
// rector_migrate.php — 検知用。8.5まで全セット ON、cosmetic だけ skip
return RectorConfig::configure()
    ->withPaths([__DIR__ . '/fuel/app', __DIR__ . '/htdocs'])  // ★ DOCROOT も対象
    ->withPhpSets(php85: true)                                  // 8.5 まで累積で全ルール
    ->withSkip([
...
```
> **走査対象に DOCROOT(`htdocs`/`public`)を必ず含める。** front controller(`index.php`)にも非互換は出る（実例: `strpos($response->body(),…)` の 8.1 null→文字列を fuel/app 限定走査で取りこぼした）。
> **`withPhpSets()` は単一バージョン指定**（`php85: true` で 8.5 まで累積。複数 true はエラー）。
...
> **8.5 で「no-op 化された結果 deprecated」になった関数（`curl_close`・`finfo_close`・`imagedestroy` 等）に注意。**
> シグネチャ不変・呼んでも動くため目視レビューでは見落としやすい。
> Rector の php85 セットは `RemoveFuncCallRector` でこれらを削除する（＝手順2を全セットで回せば取れる）。手順2を allow-list で絞ると逆に漏れるので、ここでも grep で二重に押さえる。
